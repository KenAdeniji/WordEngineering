...
public class NotHappyWithAPlaceClassXWebService : System.Web.Services.WebService
{
	...
	public const String SelectBibleWordOnlyFormat = "SELECT ScriptureReference, {0} AS VerseText FROM Bible..Scripture WHERE {0} LIKE '%{1}%' ORDER BY BookID, ChapterID, VerseID";
	public const String SelectNumberInSeriesFormat = "SELECT ScriptureReference, {0} AS VerseText FROM Bible..Scripture WHERE {0} LIKE '%{1}%' AND ({2}) ORDER BY BookID, ChapterID, VerseID";	
	public const String SelectWithNumberInFigureFormat = 
		...
		";	
	public const String SelectWithNumberInTextFormat = "SELECT ScriptureReference, {0} AS VerseText FROM Bible..Scripture WHERE {0} LIKE '%{1}%' AND {0} LIKE '%{2}%' ORDER BY BookID, ChapterID, VerseID";
