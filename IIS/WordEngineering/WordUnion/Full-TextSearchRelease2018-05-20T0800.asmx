...
public class FullTextSearch : System.Web.Services.WebService
{
	...
	{
		"SELECT ScriptureReference, {0} AS VerseText FROM Bible..Scripture WHERE CONTAINS({0}, '{1}') ORDER BY VerseIDSequence", 
		"SELECT ScriptureReference, {0} AS VerseText FROM Bible..Scripture WHERE FREETEXT({0}, '{1}') ORDER BY VerseIDSequence",
		@"	SELECT 
				FreeText_Table_KJV.ScriptureReference AS ScriptureReference,  
				FreeText_Table_KJV.{0} AS VerseText,   
				Key_Table.RANK  
			FROM 
				Bible.dbo.Scripture AS FreeText_Table_KJV INNER JOIN  
				CONTAINSTABLE ( Bible.dbo.Scripture, VerseText, '{1}' ) AS Key_Table  
		...
		FROM 
			Bible.dbo.Scripture AS FreeText_Table_KJV
		INNER JOIN  
			FREETEXTTABLE(Bible.dbo.Scripture, {0}, '{1}') AS KEY_TBL  
		ON 
